...
1. Modified the .zshrc template to include a line you need for rvm
2. Modified the .zshrc template to use the j2fly theme
3. This requires a python script in ~/bin/batcharge.py (though I don't use that anymore)
  * More details here: http://stevelosh.com/blog/2010/02/my-extravagant-zsh-prompt/

h3. The automatic installer... (do you trust me? I customized this for me specifically so be warned)

@wget --no-check-certificate https://github.com/j2fly/oh-my-zsh/raw/master/tools/install.sh -O - | sh@

h3. Make sure to install ZSH and Tree

@apt-get install zsh tree@
